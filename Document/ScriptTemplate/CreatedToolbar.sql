...
(   (SELECT MAX(STToolbarFunctionID) + 1 FROM dbo.STToolbarFunctions),  -- STToolbarFunctionID - int
    'Alive', -- AAStatus - varchar(10)
    (SELECT STToolbarID FROM dbo.STToolbars WHERE STToolbarTag = 'Complete' AND FK_STModuleID = (SELECT STModuleID FROM dbo.STModules WHERE STModuleName = 'EmployeeWorkSchedule' AND AAStatus = 'Alive')),  -- FK_STToolbarID - int
    'ActionComplete', -- STToolbarFunctionName - varchar(100)
    'void ActionComplete()', -- STToolbarFunctionFullName - varchar(255)
    'VinaERP.Modules.EmployeeWorkSchedule.EmployeeWorkScheduleModule'  -- STToolbarFunctionClass - varchar(255)
    )